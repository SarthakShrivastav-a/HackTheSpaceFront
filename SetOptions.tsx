...
import { setupReceiver, transferAsset } from './transfer.mjs';

const API_URL = 'http://10.0.2.2:3001'; // Adjust for your backend URL
...
const CreateAccount: React.FC = () => {
    ...
    const storeKeys = async (newKeypair: Keypair) => {
        try {
            const existingKeypairs = JSON.parse(await AsyncStorage.getItem('keypairs') || '[]');
            existingKeypairs.push({ publicKey: newKeypair.publicKey(), secret: newKeypair.secret() });
            await AsyncStorage.setItem('keypairs', JSON.stringify(existingKeypairs));
            setKeypairs(existingKeypairs.map((kp: any) => ({ publicKey: kp.publicKey, secret: () => kp.secret })));
    ...
    const retrieveKeys = async () => {
        try {
            const storedKeys = JSON.parse(await AsyncStorage.getItem('keypairs') || '[]');
            if (Array.isArray(storedKeys)) {
                setKeypairs(storedKeys.map((kp: any) => ({
                ...
                console.warn('Invalid keys format:', storedKeys);
            }
        } catch (error) {
    ...
    const generateKeypair = async () => {
        try {
            const response = await fetch(`${API_URL}/create-keypair`, { method: 'POST' });
            const data = await response.json();
            const newKeypair = Keypair.fromSecret(data.secret);
    ...
    const handleTransfer = async () => {
        if (keypairs.length < 3) {
            Alert.alert("Error", "Not enough accounts to transfer assets.");
            return;
        }

        const [distributor, holder, receiver] = keypairs.slice(0, 3);

        try {
            console.log("Setting up trustline for receiver...");
            await setupReceiver(receiver, distributor, "ExampleNFT");

            console.log("Trustline setup complete. Initiating transfer...");
            await transferAsset(holder, "ExampleNFT", distributor, receiver);

            console.log("Asset transfer sequence completed successfully.");
            Alert.alert("Success", "Asset transfer completed successfully.");
        } catch (error) {
            console.log("Error in asset transfer sequence:", error);
            Alert.alert("Error", "An error occurred during the transfer process.");
        }
    };
    ...
    return (
        <View style={styles.container}>
            ...
            <TouchableOpacity style={styles.button} onPress={handleTransfer} disabled={keypairs.length < 3}>
